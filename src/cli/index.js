...
function declareOptions (argv) {
  return yargs(argv)
  .usage('\x1B[1m' + npmPackage.name + '\x1B[22m - ' + npmPackage.description + '\n\nUsage: contractual [OPTIONS] [FILES]')
  .options({
    output: {
      alias: 'o',
      default: './out',
    ...
    'source-map': {
      description: 'If true generate source maps, if a string use it as the source filename for the map.'
    },
    'source-root': {
      description: 'If set, acts as the root for the source files listed in the source map.'
    },
    version: {
      alias: 'v',
